// ต่อจากบรรทัด const el = d_ ที่ขาดหายไป:
  ...
  useEffect(() => {
    ...
    const gameLoop = (now: number) => {
      ...
      const delta = Math.min((now - lastTime) / 1000, 0.1); // Cap delta เพื่อป้องกันบัคเฟรมตกแล้วทะลุแมพ
      lastTime = now;
      ...
      // อัปเดตตำแหน่ง Mesh ของผู้เล่นใน ThreeJS
      ...
      // เล่นอนิเมชันเร็วขึ้นหากกำลังอยู่ในสถานะโจมตี (ตาม Brief: "กด P เล่น Animation ไวขึ้น")
      const frameDuration = engine.animationRow === 2 ? 0.05 : 0.15; 

      if (engine.animationTimer >= frameDuration) {
        engine.animationTimer = 0;
        engine.currentFrame = (engine.currentFrame + 1) % 4; // มี 4 เฟรมต่อแถว
      }

      // คืนค่าสถานะอนิเมชันพิเศษเมื่อหมดเวลา
      ...
      // คำนวณ UV Offset ของ Sprite Sheet (4x4 Frames)
      // ตัวคูณสเกลระบุไว้แล้วที่ 0.25 ในคำสั่ง repeat.set
      const offsetX = engine.currentFrame * 0.25;
      const offsetY = (3 - engine.animationRow) * 0.25; // แถวแรกอยู่บนสุด (1.0 - 0.25 = 0.75)
      engine.playerTexture.offset.set(offsetX, offsetY);

      // จัดการ Flip ทิศทางตัวละคร ซ้าย/ขวา
      ...
      engine.ghosts.forEach(ghost => {
        if (ghost.isDying) {
          ...
          ghost.mesh.position.y += 15.0 * delta; // ปลิวขึ้นฟ้า
        ...
        // บัคป้องกัน Contact Damage ถี่เกินไป: จัดการ Tickers
        ...
        if (!ghost.isStunned) {
          ...
          if (ghost.knockbackTimer && ghost.knockbackTimer > 0) {
            ...
            // ดึงความเร็ว Knockback ลงทีละนิดให้สมูท
          ...
          // ผีตาโขนอนิเมชันแถวเดิน (Row 1 ของ enemy.png)
          ghost.ghostTexture.offset.y = 0.0;
        } else {
          // ผีโดนสะกดด้วยสกิลหมอลำปุ่ม O -> เปลี่ยนเป็นอนิเมชันเต้น (Row 0 ของ enemy.png)
          ghost.ghostTexture.offset.y = 0.5;
        }

        // แอนิเมชันเฟรมของผี (4 Columns)
        ...
        // ตรวจสอบการชนกับตัวผู้เล่น (Player Collision -> เสียชีวิต/ลดเลือด)
        if (!ghost.isStunned && !ghost.isDying) {
          ...
          if (distToPlayer < 1.4 && ghost.attackFlashTimer <= 0) {
            ghost.attackFlashTimer = 1.5; // Cooldown อมตะชั่วคราวของผู้เล่น 1.5 วินาที
            audio.playHurt();
            setLives(prev => {
      ...
      // --- 5. Update Items Falling & Collectibles ---
      engine.collectibles.forEach((item, idx) => {
        // อนิเมชันไอเทมหมุนแกน Y และลอยขึ้นลงเบา ๆ เพื่อความสวยงาม
        item.mesh.children[0].rotation.y += 1.5 * delta;
        
        // จัดการไอเทมร่วงจากฟ้า (Procedural Sky Drop)
        if ((item as any).isLanding) {
          ...
          if (item.mesh.position.y <= 0.8) {
            ...
            // แรงกระแทกพื้นสร้างฝุ่นเม็ดเล็ก ๆ
            spawnFlashParticles(item.mesh.position, new THREE.Color('#fbbf24'), 5);
          }
        }

        // ตรวจระยะเก็บไอเทม (Player Collection Radius)
        ...
        if (distToItem < 1.5) {
          ...
          engine.collectibles.splice(idx, 1);
        }
      });

      // --- 6. Update Render Particles ---
      engine.particles.forEach((p, idx) => {
        p.life -= delta;
        p.mesh.position.addScaledVector(p.velocity, delta);
        // มีแรงโน้มถ่วงกดลงเล็กน้อยสำหรับเอฟเฟกต์ระเบิดกระจาย
        p.velocity.y -= 4.0 * delta; 

        // ค่อย ๆ จางหายตาม Life time
      ...
      // --- 7. Update UI HTML Floating Texts Projection ---
      if (uiOverlayRef.current) {
        engine.floatingTexts.forEach((t, idx) => {
          t.life -= delta;
          t.y += 2.0 * delta; // ลอยขึ้นฟ้าเรื่อย ๆ

          // แปลงพิกัด 3D เวกเตอร์โลก (World Coordinates) ให้กลายเป็น 2D Screen Space พิกัดหน้าจอ
          const wp = new THREE.Vector3(t.x, t.y, t.z);
          wp.project(camera);

          // แปลงค่าช่วง [-1, 1] ไปเป็นพิกัด Pixel เปอร์เซ็นต์หน้าต่างเว็บ Browser
          ...
          if (t.life <= 0) {
            ...
            engine.floatingTexts.splice(idx, 1);
          }
        });
      }

      // เรนเดอร์ภาพสามมิติขึ้นหน้าจอ
      renderer.render(scene, camera);
    };

    animationFrameId = requestAnimationFrame(gameLoop);
    return () => cancelAnimationFrame(animationFrameId);
  }, [activeKeys, isPaused, isDead, gameStarted, isGameWon, score]);

  // ระบบสุ่มไอเทมกระหน่ำร่วงลงมาจากฟากฟ้าทุกๆ 7 วินาที
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full min-h-[500px] overflow-hidden select-none bg-slate-950">
      ...
      {/* HTML GUI Overlay for Floating Combat Damage Text */}
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start pointer-events-none z-30 font-sans">
        ...
        <div className="bg-slate-900/80 backdrop-blur-md border border-purple-500/30 p-4 rounded-xl shadow-xl flex flex-col gap-2 pointer-events-auto text-white min-w-[200px]">
          ...
          {/* พลังงาน Energy Gauge (สำหรับใช้กดปุ่ม O) */}
        ...
        {/* แผงคะแนนและผีตาโขนที่เหลือฝั่งขวา */}
        <div className="flex flex-col gap-2 items-end">
          ...
          {/* จำนวนผีที่ต้องปราบค้างอยู่ในฉาก */}
      ...
      {/* แถบแจ้งเตือนสกิลระเบิดพลังด้านล่างหน้าจอ */}
      ...
      {/* ================= MODAL OVERLAYS (WIN / PAUSE / GAME OVER) ================= */}
      {/* 1. เมนูเมื่อกดหยุดเกม (Pause Menu) */}
      {isPaused && (
      ...
      {/* 2. เมนูชัยชนะเมื่อปราบผีหมดแมพ (Victory Display) */}
      {isGameWon && (
